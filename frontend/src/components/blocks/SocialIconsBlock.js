    ...
    const handleSubmit = async () => {
        ...
        for (const link of validLinks) {
            if (!link.url.startsWith('https://')) {
                // Find platform name for better error
                const platformName = SOCIAL_PLATFORMS.find(p => p.id === link.platform)?.name || link.platform;
                setError(`Ссылка для ${platformName} должна начинаться с https://`);
                return;
            }
        }
        ...
        setLoading(true);
        try {
            const content = { links: validLinks };
            let response;
            if (block) {
