...
import { X, User, Link2, LayoutGrid, ArrowUpDown, Eye, Moon, Sun, BadgeCheck, ShieldCheck, Heart, ShoppingBag, Calendar } from 'lucide-react';
...
const EditorHelpModal = ({ onClose }) => {
    ...
    return (
        ...
        >
            <div
                className="bg-card w-full max-w-2xl h-[80vh] rounded-[20px] border border-border shadow-2xl flex flex-col overflow-hidden"
            ...
            >
                ...
                <div className="flex flex-1 overflow-hidden">
                    {/* Sidebar */}
                    <div className="w-1/3 border-r border-border bg-secondary/20 p-4 space-y-2 overflow-y-auto">
                        {tabs.map((tab) => {
                            const Icon = tab.icon;
                            return (
                                <button
                                    ...
                                    className={`w-full flex items-center gap-3 px-4 py-3 rounded-[12px] text-sm font-medium transition-all ${activeTab === tab.id
                ...
                {/* Footer */}
                <div className="p-4 border-t border-border bg-secondary/10 shrink-0 text-center">
                    <button
                        onClick={onClose}
                        className="px-8 py-2.5 bg-foreground text-background rounded-full font-bold hover:opacity-90 transition-all shadow-lg text-sm"
                    >
                        Всё понятно, спасибо!
                    </button>
                </div>
