        ...
        <div className="min-h-screen bg-background text-foreground font-sans p-6">
            ...
            <main className="max-w-7xl mx-auto space-y-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <div className="bg-card border border-border p-6 rounded-xl shadow-sm border-l-4 border-l-blue-500">
                        <div className="text-sm text-muted-foreground mb-1">Google Auth</div>
                        <div className="text-2xl font-bold text-blue-500">{stats?.registrations?.google || 0}</div>
                    </div>
                    <div className="bg-card border border-border p-6 rounded-xl shadow-sm border-l-4 border-l-green-500">
                        <div className="text-sm text-muted-foreground mb-1">Email Auth</div>
                        <div className="text-2xl font-bold text-green-500">{stats?.registrations?.email || 0}</div>
                    </div>
                </div>
                ...
                <div className="flex gap-4 border-b border-border overflow-x-auto">
                    ...
                    <button
                        ...
                        className={`px-4 py-2 text-sm font-medium border-b-2 transition-colors whitespace-nowrap ${activeTab === 'requests' ? 'border-primary text-primary' : 'border-transparent text-muted-foreground hover:text-foreground'}`}
                    >
                        Заявки ({requests.filter(r => r.status === 'pending').length})
                    </button>
                    <button
                        onClick={() => setActiveTab('verified')}
                        className={`px-4 py-2 text-sm font-medium border-b-2 transition-colors whitespace-nowrap ${activeTab === 'verified' ? 'border-primary text-primary' : 'border-transparent text-muted-foreground hover:text-foreground'}`}
                    >
                        Верифицированы ({requests.filter(r => r.status === 'approved').length})
                    </button>
                    <button
                        onClick={() => setActiveTab('cancelled')}
                        className={`px-4 py-2 text-sm font-medium border-b-2 transition-colors whitespace-nowrap ${activeTab === 'cancelled' ? 'border-primary text-primary' : 'border-transparent text-muted-foreground hover:text-foreground'}`}
                    >
                        Отмененные ({requests.filter(r => r.status === 'cancelled').length})
                    </button>
                    <button
                ...
                {(activeTab === 'requests' || activeTab === 'verified' || activeTab === 'cancelled') && (
                    <div className="bg-card border border-border rounded-xl overflow-hidden shadow-sm">
                        <div className="p-4 border-b border-border">
                            <h2 className="text-lg font-semibold">
                                {activeTab === 'requests' ? 'Новые заявки' : activeTab === 'verified' ? 'Верифицированные' : 'Отмененные заявки'}
                            </h2>
                        </div>
                        <div className="overflow-x-auto">
                            <table className="w-full text-left text-sm">
                                ...
                                <tbody className="divide-y divide-border">
                                    {(() => {
                                        const filteredRequests = requests.filter(req => {
                                            if (activeTab === 'requests') return req.status === 'pending' || req.status === 'rejected';
                                            if (activeTab === 'verified') return req.status === 'approved';
                                ...
                                <tr className="hover:bg-accent/50 transition-colors">
                                    ...
                                    <td className="px-6 py-4 text-right">
                                        <button
                                            ...
                                            className={`p-2 rounded-lg transition-colors ${savingSettings
                                                ? 'bg-muted text-muted-foreground cursor-wait'
