...
const PublicPage = () => {
  ...
  useEffect(() => {
    ...
    const connectWS = () => {
      ...
      console.log('Connecting to Live Sync WS:', wsUrl);
      ...
      ws.onmessage = (event) => {
        try {
          ...
          if (msg.type === 'page_update') {
            if (msg.data) {
              console.log('Live Sync: Instant update received');
              setData(msg.data);
            } else {
              console.log('Live Sync: Update notification received, reloading...');
              loadPage();
            }
          }
        } catch (e) {
      ...
      ws.onclose = () => {
        console.log('Live Sync: Connection closed, reconnecting in 3s...');
              ...
              {activeTab === 'profile' && linkBlocks.length > 0 && (
                <div className="space-y-3">
                  {linkBlocks.map((block) => (
                    <BlockSelector key={block.id} block={block} />
                  ))}
                </div>
              )}
            </div>
                ...
                {(() => {
                  ...
                  const flushEvents = () => {
                    ...
                    sortedEvents.forEach(block => {
                      renderedBlocks.push(<div key={block.id} className="w-full"><BlockSelector block={block} /></div>);
                    });
                    currentEventGroup = [];
                  };
